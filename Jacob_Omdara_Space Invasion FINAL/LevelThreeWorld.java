...
/**
 * This is the third world, it is the last wave that players will fight in the game. 
 * 
 */
public class LevelThreeWorld extends World
{
     ...
     */
    public LevelThreeWorld(int width,int height, int score)
    {    
    ...
    public void act()
    {
        ...
        // if there are no aliens left, then it will launch the win screen 
        if (getObjects(blackAlien.class).size() == 0)
        {
            // stops music
            musicStop();
            // it makes a new win world variable, makes it primary world
    ...
    public void setup3()
    {
        alienRows = 4; //number of rows of alien
        ...
        for(int rows = 0; rows < alienRows; rows++)
        {
            for (int columns = 0; columns < alienColumns; columns++)
            {
                ...
                Army[rows][columns] = new blackAlien (90*columns + 100, alienSpacing*rows + 100, 1 + rows, alienSpacing, Army, rows, userShip, 3, 3, 150);
