...
public class blackAlien extends Enemy
{
    ...
    {
        // if an alien is at the boundary
        if (this.getX() > getWorld().getWidth() - buffer || this.getX() < 0 + buffer)
        {
            // change the direction for the ENTIRE row of aliens
             for (int col = Army[myRow].length -1; col >= 0; col--)
            {
                Army[myRow][col].changeDirection();
            }
            
             // for (int col = 0; col > Army[myRow].length; col++)
            // {
                // Army[myRow][col].changeDirection();
            // }
        }
